...
const Notif = () => {
  let history = useHistory();
    ...
    const getnotif = async () => {
      ...
      const notiflue = async () => {
        const result = await routeApi.notiflue(token);
        if (result.ok) {
    ...
    return (
        <div className="biix" className={classes.div}>
            <div className="biix">
              ...
              <div style={{display : "flex" , justifyContent: "space-between"}} > <h5> Notifications </h5> 
              <Tooltip title="Marquer tout comme lu" placement="Right"  >
              <div><AiOutlineEye style={{marginRight :"10px"}} onClick={notiflue} /></div>
              </Tooltip>
                </div>
              {/* <button onClick={notiflue} >Marquer le tt comme lue</button> */}
                {notif && 
