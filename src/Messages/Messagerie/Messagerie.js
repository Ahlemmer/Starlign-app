import React, { useEffect, useState } from "react";
 ...
 import {BsArrowLeftCircle , BsFillCameraVideoFill, BsSearch } from 'react-icons/bs';
...
const Messagerie = () => {
  ...
  const [booli, setbooli] = useState(false);
   ...
   const decodedTokenMine = jwtDecode(token);
   ...
   useEffect( async () => {
    ...
    if (result.ok) {
      if (result.data.credentials.userId !== user1){
        history.push('/home');
  ...
  useEffect(async () => {
     ...
     const docblk = await getDoc(doc(db, "Users", user1));
     setblk(docblk.data().blk);
  ...
  g.map((i) => {
    if (bloque.indexOf(i.userId) == -1 && blk.indexOf(i.userId) == -1){
    v.push(i);
  }
  });
  
  invit.map((d) => {
    if (bloque.indexOf(d) == -1 && blk.indexOf(d) == -1){
    j.push(d);}
  });
        ...
        {chat ? (
          <>
            <div className="messages_user"  >
              ...
              onClick={()=>{ 
                if (chat.userId){
                  history.push(`/profil/${chat.userId}`);
                }else if (chat.iduser){
                  history.push(`/profil/${chat.iduser}`);
                }
                  }}> 
              <img src={chat.imageUrl} alt="avatar" className="mu_image" /> </div>
              ...
              >
                <BsFillCameraVideoFill
                  onClick={() => {
                    window.location.href = `https://calls.stralingapp.net/room/${decodedTokenMine.user_id}?receiver=${chat.userId}`;
                  }}
                  color="#19A8D9"
                  size={24}
                ...
                {/* <MdVideoCall
                  onClick={() => {
                    window.location.href = `https://calls.stralingapp.net/room/${chat.userId}`;
                  }}
                  color="#19A8D9"
                  size={32}
                  style={{
